...
Right now that we are confident with out terminology, let's think about our univariate analysis options. We know that we want to be able to talk about one variable in our data set. How can we do this?


Well remember when we spoke about **levels of measurement** last week? No? Well that was when we looked at the different variables in terms of whether they were nominal, ordinal, or numeric. If it still doesn't ring a bell, go back to last week's lab and `ctrl + f` for these terms. But hopefully you will have retained some of this. Remember we can differentiate between numeric and categorical, and then categorical we can futher sub-divide into nominal and ordinal. Here's a beautiful and scientific drawing to illustrate: 
...
So why is this important? Well what level of measurement your variable falls into dictates what types of summaries are appropriate. Thinking back to the gender example, it would not make huge amount of sense to calculate the "average gender", would it? Gender is a nominal variable, and as so an appropriate way to summarise it is not to calculate an average (mean or median, but we will get into this a bit later). Instead there are other approaches you could take. For example you could look into the modal category - which value of the variable occurs the most frequently? For example, in the prison population data above, the modal gender was male, as this was the most frequently occuring variables. But we also wanted to look at the frequency of the other values the variable can take, in this case the frequency of the female gender. So we could look at a frequency table. These are valid ways for summarising categorical variables, which may not work for numeric variables, which are better suited to measures like average and variance. But now I'm just saying words.
...
Making a frequency table in excel is quite simple, and it is achieved by using something called a **pivot table**. As far as I know this name is specific to Excel, but who knows. Anyway a pivot table is a tool that let's you build different types of summary tables from your data. One of these is a frequency table. 
...
Now when you click OK, excel should take you to the new worksheet where it has set up a pivot table for you, ready to get into your data. Now you might have to do one more step, which is to click *anywhere* inside the pivot tabe area: 
...
Now you should see all your variables on the side there as well, in this little pane that has just appeared. You can scroll through and find crime type. This is the variable we want to look at in this case. 

Now before we move to the next step, let's take a moment to understand what each of the windows in the pivot table pane mean. 
...
I hope that you can see the resemblance between the table on its side and the bar chart now!


OK so let's make our own bar chart in excel. Go back to your pivot table, that has the frequency of each crime type. Click anywhere in the table, to highlight it all. 

![](imgs/crime_type_table.png)


WHen highlighted, click on the charts tab on the top menu:
...
You can also stylize your graph. 
...
Let's create a new frequency table, this time using the 'borough' variable. I will leave you on your own to do this. You can refer back up to the steps we followed to create the crime type frequency table. But in the end you should end up with a table where there are two columns, one for 'borough' and one for 'number of crimes'. In this case, each row will be one borough. Your table will look like this: 
...
If you consider this frequency table your new data, you can see that you have two columns, which means two variables. You have one variabe for the name of each borough. And you have another one, that is the *number of burglaries*. While the borough name is a nominal variable, the *number* of burglaries is... 
...
You will often hear numeric variables summarised by the measure of central tendency. These are the **mean** and the **median**. You will have encountered the mean before, but possible referred to as the **average**. In statistical langueg you will hear people talk about *the mean number of crimes per borough is `r I(mean(per_borough_crimes$number.of.crimes))` crimes*. This is the exact same thing as talking about the average number of crimes per borough is `r I(mean(per_borough_crimes$number.of.crimes))` crimes. And you would calculate it the same exact same way. 
...
So what is the mean number of crimes per borough? You guessed it, *the mean number of crimes per borough is 42,020.8. That means that on avaerage, there are about 42,000 crimes per borough. And this measure is the mean. 


Is this a good way of describing your data? Well one way to think about it is to consider how much you distort the data if you use that measurement to talk about it. Normally when we think about average, we think that this is a measure that represents a value somewhere in the middle. But if we look at this value, *42,020.8*, we see that actually this number is higher than almost all the boroughs. There is only one borough with a number of crimes that is higher than the average. All the other boroughs have below average crime rates. Why do you think this is?
...
What you are seeing in the histograms above are the **distributions** of the scores that are given to each film on the IMDB website. It is possible to have a look at the distribution of the number of crimes per borough as well. 
...
This graph is called a **histogram**. If it does't quite make sense, have a look at 
...
So how do you build a histogram? Of course I won't make you draw one manually, every time you need to build one, so let's get to excel and make our own [put in here histo with data analsis toolpak]

Note: At this point it's worth to stop and think about the difference between a histogram. Take a moment to discuss with the person next to you 
...
You can set the last parameter of all the values, from 0 to 4, in order to produce all the 5 numbers, minimum, 1st quartile, median, 3rd quartile, and maximum. The 


##Mean in Excel

Now that you understand how the mean and median are calculated, what they reprepsent, and what situations are best to each one of them in, let's move on to the practicalities of how you calculate these in excel. It's actually very easy, so hopefully you will get the hang of it very quickly. 
...
If you want to go ahead and learn how to build one in excel, you can do so by folowing the tutorial below: 

- [Make a boplot in excel](https://support.office.com/en-gb/article/Create-a-box-plot-10204530-8cdf-40fe-a711-2eb9785e510f)
...
- Standard deviation: `r I(sd(per_borough_crimes$number.of.crimes)))`
...
The standard deviation for our data is `r I(sd(per_borough_crimes$number.of.crimes))`. But, wait a minute! That is a bit large, isn’t it? How is it that half the boroughs have a value below `r I(quantile(per_borough_crimes$number.of.crimes)[3])`, the mean is `r I(mean(per_borough_crimes$number.of.crimes))`, and you are telling me the standard deviation, in other words, the average distance of each individual value to the mean is `r I(sd(per_borough_crimes$number.of.crimes))`? This makes no sense? Shouldn’t the standard deviation be smaller, given that most countries should be close to the mean? 
