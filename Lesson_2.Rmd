...
[Make barchart in excel steps here]
...
So how do you build a histogram? Of course I won't make you draw one manually, every time you need to build one, so let's get to excel and make our own using chart builder. 
...
One way of thinking about spread is to examine how far each data value is from the mean. This difference is called a deviation. If we look at our dataset we see that Albania had 4 homicides per 100,000 inhabitants in 2010. If the mean for all the countries is 10.32 then Albania has a deviation of 6.32 (=10.32-4).
...
10.32094	13.35499 	13.3	0	1.5	4.2	14.8	91.6	191	3
The standard deviation for our data is 13.35. But, wait a minute! That is a bit large, isn’t it? How is it that most people have a value below 4.2, the mean is 10.32, and you are telling me the standard deviation, in other words, the average distance of each individual value to the mean is 13.35? This makes no sense? Shouldn’t the standard deviation be smaller, given that most countries should be close to the mean? 
...
This is actually quite simple. Just open up a word document. Then return to your pivot table, and highlight the whole thing. You can highlight all in a few ways. One method is to click on the bottom right cell, and just drag the mouse over to the top left cell. When you let go of the mouse, everything will be highlighted. Alternatively you can also click on any cell inside your pivot table and hit `Ctrl+A` on a PC, or `cmd+A` on a mac. 
